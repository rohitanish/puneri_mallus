...
interface EventCardProps {
  eventId?: string; // 🔥 REQUIRED FOR INTERNAL ROUTING
  title: string;
  date: string;
  image: string;
  category: string;
      ...
      <div className={`relative h-full flex flex-col overflow-hidden rounded-[45px] bg-zinc-950/40 backdrop-blur-xl border transition-all duration-700 ${
        ...
        <div className="p-8 md:p-10 flex-1 flex flex-col relative z-20">
          ...
          <div className="mt-auto space-y-4">
            ...
            {/* 🔥 UPDATED SECURE SPOT BUTTON */}
            {isUpcoming && (
              <Link 
                href={ticketUrl === 'INTERNAL' && eventId ? `/events/${eventId}/book` : (ticketUrl || "/events")} 
                target={ticketUrl === 'INTERNAL' ? "_self" : "_blank"} 
                rel={ticketUrl === 'INTERNAL' ? "" : "noopener noreferrer"} 
